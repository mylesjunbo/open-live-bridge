...
constexpr const char *kVideoInputDeviceCategory =
...
bool QueryRegistryString(HKEY root, const std::string &subkey,
                         const char *valueName, REGSAM view,
                         std::string *value) {
  HKEY key = nullptr;
  if (RegOpenKeyExA(root, subkey.c_str(), 0, KEY_READ | view, &key) !=
      ERROR_SUCCESS) {
    return false;
  }
  ...
  DWORD type = 0;
  DWORD size = 0;
  if (RegQueryValueExA(key, valueName, nullptr, &type, nullptr, &size) !=
          ERROR_SUCCESS ||
      size == 0) {
    RegCloseKey(key);
    return false;
  ...
  const LONG result =
      RegQueryValueExA(key, valueName, nullptr, &type,
...
void QueryView(const std::string &clsid, REGSAM view, bool *registered,
               std::string *friendlyName, std::string *modulePath) {
  const std::string comKey = std::string(kClsidPrefix) + clsid;
  const std::string inprocKey = comKey + "\\InprocServer32";
  const std::string categoryKey = std::string(kClsidPrefix) +
                                  kVideoInputDeviceCategory + "\\Instance\\" +
                                  clsid;
  ...
  const bool hasCategoryName = QueryRegistryString(
      HKEY_LOCAL_MACHINE, categoryKey, "FriendlyName", view, &categoryName);
  const bool hasComName =
      QueryRegistryString(HKEY_LOCAL_MACHINE, comKey, nullptr, view, &comName);
  const bool hasModulePath =
      QueryRegistryString(HKEY_LOCAL_MACHINE, inprocKey, nullptr, view, &path);
...
bool VirtualCameraIdentity::HasClsid() const { return !clsid.empty(); }

bool QueryVirtualCameraRegistration(const VirtualCameraIdentity &identity,
  ...
  *status = {};
  ...
  QueryView(normalizedClsid, KEY_WOW64_64KEY, &status->registered64,
            &status->friendlyName64, &status->modulePath64);
  QueryView(normalizedClsid, KEY_WOW64_32KEY, &status->registered32,
            &status->friendlyName32, &status->modulePath32);
  return true;
}

std::string
RegistrationStatusToJson(const VirtualCameraIdentity &identity,
